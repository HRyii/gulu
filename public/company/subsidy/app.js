$(document).ready(function() {
    ...
    $(".gulu-js-company-list").on("click", ".gulu-js-btn-delete", function() {
        var $that = $(this);
    ...
    $(".gulu-subsidy-list").on("change", "input[name='enable']", function() {
        var $that = $(this);
        $.post('/api/company.subsidy/status?id=' + $that.data('id'), {status: $that.val()}, function(res) {
            if (res.code === 0) {
                toastr.success(res.text);
            }
        }, 'json');
...
$(".gulu-js-company-list .gulu-js-btn-check").on("click", function() {
    var $that = $(this);
    layer.confirm("确定审核?", {shade: false}, function() {
        $('.layui-layer').hide();
        var id = $that.siblings('img').data('id');
        $.post('/api/compamy.Corporate/check', {id: id}, function(res) {
            if (res.code === 0) {
