$(document).ready(function() {
    ...
    $(".yele-js-brand-list").on("click", ".yele-js-btn-delete", function() {
        var $that = $(this);
    ...
    var id = [];
    $(".yele-js-brand-list").on("ifChanged", ".i-checks.yele-js-check", function() {
        if ($(this).is(':checked')) {
    ...
    $(".yele-js-btn-delete-all").on("click", function() {
        layer.confirm("确定删除?", {shade: false}, function() {
            $.post('/api/goods.Brand/delete', {id: id}, function(res) {
                if (res.code === 0) {
                    document.location.href = '/goods/Brand/index';
                }
            }, 'json');
        });
    });
