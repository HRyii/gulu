$(document).ready(function() {
    ...
    $(".yele-js-accounts-list").on("click", ".yele-js-btn-delete", function() {
        var $that = $(this);
    ...
    $(".yele-js-accounts-list").on("change", "select[name='enabled']", function() {
        var $that = $(this);
    ...
    var id = [];
    $(".yele-js-accounts-list").on("ifChanged", ".i-checks.yele-js-check", function() {
        if ($(this).is(':checked')) {
    ...
    $(".yele-js-btn-delete-all").on("click", function() {
        layer.confirm("确定删除?", {shade: false}, function() {
            $.post('/api/account.user/delete', {id: id}, function(res) {
                if (res.code === 0) {
                    document.location.href = '/account/user/index';
                }
            }, 'json');
        });
    });
