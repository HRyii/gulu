            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title" style="padding: 16px;min-height: 60px;">
                        <h5>商品仓库</h5>
                        {if condition="$__user->can(['goods_store_create'])"}
                        <a class="label label-primary pull-right" href="{:url('goods/store/create')}" style="padding: 11px 8px !important;"><i class="fa fa-pencil-square-o"></i><span class="bold">&nbsp;创建</span></a>
                        {/if}
                        {if condition="$__user->can(['goods_store_delete'])"}
                        ...
                        <form name="searchform" method="post">
                            <div class="input-group" style="width:15%; float:right;">
                                <input type="text" class="form-control" name="keyword" value="{$keyword}" placeholder="请输入仓库关键字"> <span class="input-group-btn"> <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i>&nbsp;搜索
                                </button> </span>
                            </div>
                        </form>
                    </div>
                    <div class="ibox-content">
                        ...
                        {/if}
                        <table class="table table-striped yele-js-store-list">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                    <input type="checkbox"  data-id="{$store['id']}" class="i-checks yele-js-check"></input>
                                    </td>
                                    <td>{$store['name']}</td>
                                    <td>
                                    <form action="{:url('goods/store/delete', ['id' => $store['id']])}" method="post">
                                        <div class="form-group">
                                            {if condition="$__user->can(['goods_store_update'])"}
                                            <a href="{:url('goods/store/update', ['id' => $store['id']])}" class="btn btn-sm btn-success"><i class="fa fa-paste"></i><span class="bold">&nbsp;编辑</span></a>
                                            {/if}
                                            {if condition="$__user->can(['goods_store_delete'])"}
                ...
                {if condition="!($search == 1)"}{$stores->render()}{/if}
            </div>
        </div>
    </div>
    {/block}
